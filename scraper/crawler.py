...
async def get_links(page):
    ...
    try:
        await page.wait_for_selector('div[data-job-id]', timeout=5000)
    except:
        print("Không còn job ở trang này")
        await page.wait_for_timeout(random.choice([5000]))
        return []
...
async def get_job(page, link):
    try:
        ...
        print(f"Lỗi khi lấy thông tin job từ link {link}: {e}")
        return None
    return {
        "link": link,
        "title": job_title,
...
async def get_all_links(page):
    ...
    for start in range(0, total_jobs, 25):
        try:
            ...
            print(f"Lỗi khi lấy links từ trang {start//25 + 1}: {e}")
