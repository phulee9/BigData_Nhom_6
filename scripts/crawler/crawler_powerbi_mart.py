...
def silver_weekly_object(week: str) -> str:
    return f"{SILVER_WEEKLY_PREFIX}{week}.parquet"


def list_silver_weeks(client) -> list[str]:
...
def find_unprocessed_week(client, engine) -> str | None:
    silver_weeks = list_silver_weeks(client)
    processed_weeks = get_processed_weeks(engine)
